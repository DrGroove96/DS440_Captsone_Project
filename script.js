...
async function updateForecastData(lat, lon) {
    try {
        ...
        // Process user reported temperatures for visualization
        const userTemps = hourlyLabels.map((label, index) => {
            // Find all temperatures reported in this hour
            const hoursAgo = (currentHour - parseInt(label.split(':')[0]) + 24) % 24;
            const relevantTemps = userReportedTemps.filter(temp => {
                const tempHour = temp.timestamp.getHours();
                return tempHour === (currentHour - hoursAgo + 24) % 24;
            });

            // If we have temperatures for this hour, return the average
            if (relevantTemps.length > 0) {
                const sum = relevantTemps.reduce((acc, curr) => acc + curr.temperature, 0);
                return sum / relevantTemps.length;
            }
            return null;
        });

        console.log('User reported temperatures for chart:', userTemps);
