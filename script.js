...
async function updateForecastData(lat, lon) {
    try {
        ...
        // Process daily data for 5-day forecast
        const dailyData = data.list.filter((item, index) => index % 8 === 0).slice(0, 5);
        const dailyLabels = dailyData.map(item => formatDate(item.dt));

        // Process user reported temperatures
        const userTemps = hourlyLabels.map(label => {
            const hour = parseInt(label.split(':')[0]);
            const matchingTemps = userReportedTemps.filter(temp => 
                temp.timestamp.getHours() === hour
            );
            if (matchingTemps.length > 0) {
                return matchingTemps.reduce((sum, temp) => sum + temp.temperature, 0) / matchingTemps.length;
            }
            return null;
        });
        ...
        if (hourlyTempChart) {
            hourlyTempChart.data.labels = hourlyLabels;
            hourlyTempChart.data.datasets = [
                {
                    label: 'API Temperature (°C)',
        ...
        if (dailyTempChart) {
            dailyTempChart.data.labels = dailyLabels;
            dailyTempChart.data.datasets = [{
                label: '5-Day Temperature (°C)',
        ...
        if (precipitationChart) {
            precipitationChart.data.labels = hourlyLabels;
            precipitationChart.data.datasets = [
                {
                    label: 'Rain (mm)',
                    data: hourlyData.map(item => item.rain ? item.rain['3h'] || 0 : 0),
                    backgroundColor: 'rgba(54, 162, 235, 0.5)'
                },
                {
                    label: 'Snow (mm)',
                    data: hourlyData.map(item => item.snow ? item.snow['3h'] || 0 : 0),
                    backgroundColor: 'rgba(201, 203, 207, 0.5)'
                }
            ];
        ...
        console.log('Charts updated with:', {
            hourlyLabels,
            userTemps,
            dailyLabels
        });
...
function switchForecast(type) {
    const hourlyContent = document.getElementById('hourlyForecast');
    const dailyContent = document.getElementById('dailyForecast');
