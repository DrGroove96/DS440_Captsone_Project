...
async function updateForecastData(lat, lon) {
    try {
        ...
        // Match user reported temperatures with forecast times
        const userTemps = hourlyLabels.map(label => {
            const hour = parseInt(label.split(':')[0]);
            const matchingTemp = userReportedTemps.find(temp => 
                temp.timestamp.getHours() === hour
            );
            return matchingTemp ? matchingTemp.temperature : null;
        });
...
function listenForUserTemps() {
    ...
    onValue(userTempsRef, (snapshot) => {
        ...
        Object.values(data).forEach(entry => {
            if (entry.latitude === currentLocation.lat && entry.longitude === currentLocation.lon) {
                userReportedTemps.push({
        ...
        // Update forecast data to refresh charts
        updateForecastData(currentLocation.lat, currentLocation.lon);
    });
}
